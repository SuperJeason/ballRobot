    ...
    def check_new_task(self, new_task):
        with self.task_lock:
            if not self.current_task or new_task.priority < self.current_task.priority:
                if self.current_task:
                    print(f"发现更高优先级任务: {new_task.name}，中断当前任务: {self.current_task.name}")
...
def yolov8_detection_thread(detection_result, stop_event):
    ...
    while not stop_event.is_set():
        ...
        task_id = detect_task_from_frame(frame, model)
        if task_id:
...
def create_task(task_id):
    if task_id == 1:
        return Task("最低优先级，放对面有一个球的框", 1, task_function)
    elif task_id == 2:
        return Task("放没有球的框", 2, task_function)
    elif task_id == 3:
        return Task("放已经有两个本方球的框", 3, task_function)
    elif task_id == 4:
        return Task("有一个对方球一个本方球的框", 4, task_function)
    return None
...
if __name__ == "__main__":
    ...
    detection_result = {"task_id": None}
    yolov8_stop_event = threading.Event()

    # 启动YOLOv8检测线程
    yolov8_thread = threading.Thread(target=yolov8_detection_thread, args=(detection_result, yolov8_stop_event))
    yolov8_thread.start()
    ...
    while not task_manager.stop_event.is_set():
        if detection_result["task_id"] is not None:
            new_task = create_task(detection_result["task_id"])
            if new_task:
